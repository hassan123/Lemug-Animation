 ...
 */
	...
	// @constants
	//------------------------
	URL:'http://localhost/Lemug/Web-Development/',
	...
	$home: {
		bloc1 : {h1:$('#home-bloc1 h1'),h2:$('#home-bloc1 h2 '),me:$('#home-bloc1')},
		bloc3 : $('#home-bloc3'),
	...
	bikeHere: false,
	indicator :  {selector:$('#menu-slider .bar'),multiplier:10/8,divisor:8/10},
	//Timers
	...
	crossBrowserTranslate: function(el,x,y){
		if(Lamoulade.Support.transform){
			Normalize.transform(el, 'translate3d(' + x + 'px,' + y + 'px,0)');
		}
	},

	moveHomeBlockDown: function(pos){
			Lamoulade.$home.bloc3.css({left:-pos * Lamoulade.Multiplier * 2.5 +'px'});
			if(pos >4.5){
	...
	moveHomeBlocksUp :function(pos){
			...
			if(pos <= 0){
				Lamoulade.$home.bloc3.css({left:0+'px'});
	...
	moveBikeDown: function(pos){
			if(!Lamoulade.bikeHere){
						Lamoulade.$projects.title.container.show();
						Lamoulade.$projects.title.container.css({opacity:0,left:"48%"});
						Lamoulade.$projects.title.container.animate({opacity:1,left:50+"%"},600);
				...
				switch(position){
					case 0:
						Lamoulade.$projects.title.bike.css({'background-position-x':0,'background-position-y':0});
						break;
					case 1:
						Lamoulade.$projects.title.bike.css({'background-position-x':'-190px','background-position-y':0});
						break;
					case 2:
						Lamoulade.$projects.title.bike.css({'background-position-x':'-380px','background-position-y':0});
						break;
					case 3:
						Lamoulade.$projects.title.bike.css({'background-position-x':0,'background-position-y':'-100px'});
						break;
					case 4:
						Lamoulade.$projects.title.bike.css({'background-position-x':'-190px','background-position-y':'-100px'});
						break;
					case 5:
						Lamoulade.$projects.title.bike.css({'background-position-x':'-380px','background-position-y':'-100px'});
						break;
				}
	...
	newControls: function(pos,delta)
	{
		...
		$(Lamoulade.indicator.selector).css({'width': 75+pos*Lamoulade.indicator.multiplier});
		...
		if(delta>0){
			...
			if(pos > 123){
				Lamoulade.$projects.title.container.addClass('onlybike');
			}else{
				Lamoulade.$projects.title.container.removeClass('onlybike');
			}
			if(pos > 173){
				Lamoulade.$projects.title.container.addClass('invisible');
			}else{
				Lamoulade.$projects.title.container.removeClass('invisible');
			}
			if(pos < 183){
    	...
    	if(pos<15 && Lamoulade.CHAPTER !== Lamoulade.CHAPTERS.home){
			...
			if(pos<50){
				Lamoulade.$projects.title.container.hide();
			}
			...
			if(pos <=92){
				...
				if(Lamoulade.bikeHere){
					...
					Lamoulade.$projects.title.container.css({opacity:1,left:"50%"});
					Lamoulade.$projects.title.container.stop().animate({opacity:0,left:50+"%"},200);
					Lamoulade.bikeHere =false;
					//var veloSound = document.getElementById('velo-sound');
					//veloSound.play();
				}
			...
			if(pos >92){
				Lamoulade.$services.hide();
				Lamoulade.$projects.title.container.css({opacity:1});
				var counting = pos-92;
				var position = parseInt(counting%6,10);
  ...
  $('#home-bloc3 .menu-elem').live('click',function(e){
		e.preventDefault();
		e.stopPropagation();
...
var lamoulade = Lamoulade._init();
			$('footer').live({mouseenter:function(){
			...
			$.ajax({
				...
				success:function(data){
					...
					Lamoulade.$projects = {
						...
						title: { container: $('#chapter-title'), bike: $('#chapter-title .bike') },
